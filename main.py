...
class PlainObj(nn.Module):
    ...
    R, T, CAD_model, gt_seg, gt_dpt, gt_pcd, ... -> losses
    transformed_v = R v + T
    ...
    """
    def __init__(self, rotvec, trans, objmesh, objseg, handMask, objdpt, pcd, camMat, crop=(0,1080,0,1920), size=1920):
...
if __name__ == '__main__':
    from loadfile import read_mask2bbox, read_rt, read_mask
    cuda_device = 'cuda:0'
    mesh = o3d.io.read_triangle_mesh('car36.obj')
    depth2d = cv2.imread('0.png', cv2.IMREAD_UNCHANGED)
    camMat = np.load('color_intrin.npy')
    anno_path = '0.json'
    mask_path = '00000.png'
